...
def WorkflowStep_Planner_node(state: AgentState):
...
def WorkflowStep_Executor_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
workflow.add_node("WorkflowStep_Planner", WorkflowStep_Planner_node)

workflow.add_node("WorkflowStep_Executor", WorkflowStep_Executor_node)
