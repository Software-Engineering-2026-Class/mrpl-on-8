...
def WriteEmailStep_node(state: AgentState):
...
def InterruptStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
def RewriteEmailStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
def SendEmailStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
workflow.add_node("WriteEmailStep", WriteEmailStep_node)

workflow.add_node("InterruptStep", InterruptStep_node)

workflow.add_node("RewriteEmailStep", RewriteEmailStep_node)

workflow.add_node("SendEmailStep", SendEmailStep_node)


# Add Entry Points (START)

workflow.add_edge(START, "WriteEmailStep")

workflow.add_edge(START, "RewriteEmailStep")
