...
def PrepareStep_node(state: AgentState):
...
def WriterStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
def SuggestionsStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
workflow.add_node("PrepareStep", PrepareStep_node)

workflow.add_node("WriterStep", WriterStep_node)

workflow.add_node("SuggestionsStep", SuggestionsStep_node)


# Add Entry Points (START)

workflow.add_edge(START, "PrepareStep")

workflow.add_edge(START, "WriterStep")
