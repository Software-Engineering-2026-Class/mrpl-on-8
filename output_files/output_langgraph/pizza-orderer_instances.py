...
def step_findStore_node(state: AgentState):
...
def step_orderPizza_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
workflow.add_node("step_findStore", step_findStore_node)

workflow.add_node("step_orderPizza", step_orderPizza_node)
