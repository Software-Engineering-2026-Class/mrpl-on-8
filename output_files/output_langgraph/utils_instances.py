...
def CapitalizeSentenceStep_node(state: AgentState):
...
def CapitalizeStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
def FormatMessagesStep_node(state: AgentState):
    '''Executes the task for GenericAgent.'''
    system_msg = SystemMessage(content="Execute task.")
    response = llm.invoke([system_msg] + state["messages"])
    return {"messages": [response]}
...
workflow.add_node("CapitalizeSentenceStep", CapitalizeSentenceStep_node)

workflow.add_node("CapitalizeStep", CapitalizeStep_node)

workflow.add_node("FormatMessagesStep", FormatMessagesStep_node)
